...
void Adafruit_StepperMotor::killPins(int left, int right, int kill){
  leftSwitch = left;
  rightSwitch = right;
  killSwitch = kill;
}

int Adafruit_StepperMotor::step(uint16_t steps, uint8_t dir,  uint8_t style) {
  ...
  while (steps--) {
    ...
    if(digitalRead(killSwitch) == 1){
      return steps;
    } else if(dir == FORWARD && digitalRead(rightSwitch) == 0){
        return steps;
    } else if(dir == BACKWARD && digitalRead(leftSwitch) == 0){
        return steps;
    }
...
void Adafruit_StepperMotor::forceStep(uint16_t steps, uint8_t dir,  uint8_t style) {

  for(int k = 0; k < steps; k++){

    if(digitalRead(killSwitch) == 0){
      onestep(dir, style);  
    }
